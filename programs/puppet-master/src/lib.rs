...
mod puppet_master {
    ...
    pub fn pull_strings(ctx: Context<PullStrings>, bump: u8, data: u64) -> Result<()> {
        ...
        msg!(&format!("puppet_master prog pull_strings metod  puppet value  : {:?}", ctx.accounts.puppet));
        msg!(&format!("puppet_master prog pull_strings metod authority value : {:?}", ctx.accounts.authority));
        // let bump = &[bump][..];
        let signer_seeds: &[&[&[u8]]] = &[&[
            b"puppet",
            &[bump],
    ...
    pub fn initialize_extra_account(ctx: Context<InitializeExtraAccountMeta>) -> Result<()> {
        ...
        msg!(&format!("puppet_master prog initialize_extra_account metod  payer value  : {:?}", ctx.accounts.payer));
